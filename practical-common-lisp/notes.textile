...
* self-evaluating object: when given to the evaluator, evaluates to
  itself. Examples: 8, nil
* format: easiest way is with (format t "string"). Required arguments:
...
* Property list: every other element is a symbol that describes what
  the next element in the list is.
  Ex: (list :a 1 :b 2 :c 3)
